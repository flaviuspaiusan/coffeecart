...
let currentMenu = getMenuItems();

// Auto-sort existing localStorage to match desired order
const orderMap = {
    'espresso': 1,
    'presso': 2,
    'cortado': 3,
...
const originalOrder = currentMenu.map(i => i.id).join(',');
currentMenu.forEach((item, index) => item._origIdx = index);
...
currentMenu.forEach(item => delete item._origIdx);

if (currentMenu.map(i => i.id).join(',') !== originalOrder) {
    localStorage.setItem('coffee_menu', JSON.stringify(currentMenu));
}

if (!localStorage.getItem('coffee_menu')) {
    localStorage.setItem('coffee_menu', JSON.stringify(defaultMenuItems));
}
...
function openModal(itemId) {
    ...
    if (espressoOptions) {
        espressoOptions.style.display = itemId === 'espresso' ? 'block' : 'none';
    }
    ...
    // Ascunde butonul de confirmare daca comenzile sunt dezactivate
...
checkoutForm.addEventListener('submit', (e) => {
    ...
    if (itemId === 'espresso') {
        ...
        if (typeEl) {
            finalItemName = `Espresso ${typeEl.value}`;
        }
    }

    const aromaCheckbox = document.getElementById('aroma-caramel');
    if (aromaCheckbox && aromaCheckbox.checked) {
        finalItemName += ` + ${aromaCheckbox.value}`;
    }
    ...
    localStorage.setItem('coffee_orders', JSON.stringify(existingOrders));
    localStorage.setItem('my_active_order_id', order.id);
    
    // Dispatch a custom event so other tabs know (storage event works across tabs natively, but just to be sure)
        ...
        if(activeOrderBanner) {
            ...
            activeOrderBanner.innerHTML = `
                ...
                <div class="active-order-details">
                    Produs: <strong>${myOrder.itemName}</strong> | Nume: ${myOrder.customerName}
                </div>
                <div class="queue-count" style="color: #2e7d32; border: 1px solid rgba(46, 125, 50, 0.3);">
                    Te rugăm să o ridici. O zi frumoasă!
                </div>
            `;
        }
    } else {
...
// Face ca functia clearActiveOrder sa fie in global scope pentru butonul din HTML
...
window.addEventListener('storage', (e) => {
    ...
    if (e.key === 'coffee_menu') {
        renderMenu();
    }
});

setInterval(renderActiveOrder, 5000);
